@extends('layouts.app')
@section('title') Add New Hotspot Zone @endsection
...
<style>
    ...
    /* divider */
    .hz-divider { margin:24px 0; border:none; border-top:1px solid var(--hz-border); }
    ...
    .hz-btn-submit {
        ...
        background:linear-gradient(135deg,var(--hz-primary),var(--hz-primary-d));
        color:#fff; font-size:.9rem; font-weight:700; border:none;
        cursor:pointer; transition:transform .18s, box-shadow .18s;
        box-shadow:0 4px 14px rgba(103,119,239,.35);
    }
    .hz-btn-submit:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(103,119,239,.5); }
    .hz-btn-cancel {
        display:inline-flex; align-items:center; gap:8px;
    ...
    /* status toggle pills */
    .hz-toggle-group { display:flex; gap:8px; flex-wrap:wrap; }
    .hz-toggle-label {
        display:flex; align-items:center; gap:6px;
        ...
        color:var(--hz-muted); transition:all .2s;
    }
    .hz-toggle-label input { display:none; }
    .hz-toggle-label:has(input:checked) {
        border-color:var(--hz-primary);
        background:rgba(103,119,239,.1);
        color:var(--hz-primary);
    }
    .hz-toggle-label.success:has(input:checked) {
        border-color:var(--hz-success);
        background:rgba(71,195,99,.1);
        color:#28a745;
    }
    .hz-toggle-label.danger:has(input:checked) {
...
<div class="hz-form-page">
    ...
    <div class="hz-form-header">
        <div class="hz-form-heading">
            <div class="hz-form-icon"><i class="fas fa-plus-circle"></i></div>
            <div>
                <h1>Add New Hotspot Zone</h1>
                <p>Register a new network zone with device information</p>
            </div>
        </div>
            ...
            <div class="hz-field-grid cols-3">
                <div class="hz-field">
                    <label class="hz-label">
                        <i class="fas fa-hashtag"></i> Zone ID <span class="req">*</span>
                    </label>
                    {!! Form::text('zone_id', null, ['class'=>'hz-input','placeholder'=>'e.g. HZ-001']) !!}
                </div>
                <div class="hz-field" style="grid-column:span 2;">
                    <label class="hz-label">
                        <i class="fas fa-map-marker-alt"></i> Zone Title <span class="req">*</span>
                    </label>
                    {!! Form::text('zone_title', null, ['class'=>'hz-input','placeholder'=>'e.g. North Campus Zone A']) !!}
                </div>
            </div>
        </div>
        ...
        <div class="hz-form-body" style="padding-top:20px;padding-bottom:0;">
            <div class="hz-field-grid cols-2">
                ...
                <div class="hz-field">
                    ...
                    {!! Form::text('device_mac', null, ['class'=>'hz-input','placeholder'=>'e.g. AA:BB:CC:DD:EE:FF']) !!}
                </div>
                <div class="hz-field">
        ...
        <div class="hz-form-body" style="padding-top:20px;padding-bottom:0;">
            <div class="hz-field-grid cols-2">
                ...
                <div class="hz-field">
                    ...
                    {!! Form::text('onu_mac', null, ['class'=>'hz-input','placeholder'=>'e.g. AA:BB:CC:DD:EE:FF']) !!}
                </div>
            </div>
        </div>
            ...
            <div class="hz-field-grid cols-4">
                <div class="hz-field">
                    ...
                    <div class="hz-select-wrap">
                        {!! Form::select('card_seller', $card_sellerItems, null, ['class'=>'hz-select','placeholder'=>'Select seller']) !!}
                    </div>
                </div>
                <div class="hz-field">
